...
NEUROTRACE_CONTRACT = ProjectContract(
    slug="neurotrace-it",
    required_docs=(
        ...
        "reports/schema/trajectory_signature_manifest.json",
        "reports/experiment_plan/endpoint_baseline_plan.md",
    ...
    required_config_markers=(
        ...
        "gates:",
        "authorized: false",
        "trajectory_endpoint_relative_gain:",
...
def validate_manifest(manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if manifest.get("project") != NEUROTRACE_CONTRACT.slug:
        errors.append("project slug must be neurotrace-it")
    ...
    if "endpoint_neuron_selection" not in manifest.get("baselines", []):
        errors.append("endpoint neuron selection baseline is required")
    return errors


def required_paths(root: Path) -> list[Path]:
    return [root / item for item in NEUROTRACE_CONTRACT.required_docs]
